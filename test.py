...
import draftrulesconfig as config
	...
	while rownum < worksheet.nrows:
		...
		#Extracts rule data and updates rule
			...
			if rule_set == 'Prescreen':
				...
				dp = browser.find_element_by_xpath("//*[contains(text(), 'Decision Profiles')]")
				dp.click()
				print(rule_set)
				bld = browser.find_element_by_xpath("//button[@ng-click='buildAndDeploy(tablerow.internalProfileId)']")
				bld.click()
				print(rule_set)
...
def update_rule(rule_name, rule_value):
	...
	print(len(field_inputs))
	if rule_name == 'OutstandingUnsecuredRevolvingDebt-Maximum':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[2].value']")
		inputBox.clear()
		new_input = "{}*${{applicants[0].currentEmployments[0].grossIncome}}".format(rule_value)
		inputBox.send_keys(new_input)
		...
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[2].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value[j])
	...
	elif rule_name == 'LTV-UsedVehicle':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value)		
			
	elif rule_name == 'LTV-NewVehicle':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value)
	
	elif rule_name == 'MaximumAgeofVehicle':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value)
	
	elif rule_name == 'MinimumAmounttoFinance':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value)
		
	elif rule_name == 'PendingReferRule':
		inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		inputBox.clear()
		inputBox.send_keys(rule_value)	
	
	elif rule_name == 'Numberof306090HistoricalDelinquencies':
		build_rules() 
		#inputBox = browser.find_element_by_xpath("//input[@ng-model='condition.values[1].value']")
		#inputBox.clear()
		#inputBox.send_keys(rule_value)
